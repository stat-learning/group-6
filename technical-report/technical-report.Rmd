...
We can observe from the image that 1-0 pair is well seperated, 4-9 pair is overlapping. Based on the distribution of digits, we think PC1 represents centerization of the digit, for example, 0 has more pixels on the outer area of 28 by 28 box. 1 has more pixels in the middle which can be illustrated from the mean images above. Thus, generally 0s has larger PC1 than 1s. We think PC2 is related to horizontal symmetry, for example, 3 and 1 are symmetric horizontally, which have larger PC2. 9 and 7 are not horizontally symmetric, which have lower PC2. 


Based on the PC graph above, we predict that 1-0 pair will be easily recognized and 4-9 pair might be the hardest one to seperate. 
