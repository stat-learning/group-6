...
As shown by the frequency graph above, each digit has a roughly equal amount of input images. The digits are well-separated.  


We can see that the digits are centered and manipulated with 
some anti-aliasing technique (Figure 2). Among six 1's below, different styles appear such as italic version and  formal version. We can say that the data set has generalization of digit styles. 
...
For the PCA-reduced dataset, we also observed that the error rate gradually flatlines when the best number of trees exceeds 50. The random forest considered all 20 PCs to be important predictors, as none of which was scored 0 in both the mean decrease accuracy and the mean decrease gini plots. The random forest identified 7-9 principle components as more important than others, among which PC1, PC2 and PC4 are considered the most important (Figure 12).
...
\hline \\ [-1.5ex]
Model & Original dataset & PCA-reduced dataset & Published results \\ [1ex]
\hline \\ [-1.5ex]
