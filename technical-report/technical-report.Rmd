...
classifcation rate is produced by convolutional neural nets, which is 99.77%. We woud like to build classifiers, using our knowledge from this course, to recognize what digit (0-9) is displayed in a given image. We are interested in seeing how our models would perform with respect to literature results.
...
We can think of this as an automated way of creating polynomial terms and interaction terms in logistic regression. We chose the "radial" kernel (RBF or Gaussian) and achieved a **2.17%** misclassification rate, which is our best result so far. Below is the plot of the class regions for the SVM model using the first two PCs as the predictors. 

```{r fig.cap="Visualization of SVM.", fig.align="center", echo=FALSE}
knitr::include_graphics('images/svm.png', dpi = 120)
```

The built-in `plot()` method did not give us a clear visualization with 10 different classes and 20 predictors, so we plot the decision boundary with only 2 predictors. We could plot this model with other predictors, which might generate different decision boundaries, but that would be redundant to illustrate the model. If PC1 and PC2 are adequate to train a classifier with good predictive power and we train our model with PC1 and PC2, we suppose the plot above will have similar pattern as the PCA plot we get in earlier section. 
...
```{r fig.cap="Tree plot on the original dataset from the `rpart` package.", fig.align="center", echo=FALSE}
knitr::include_graphics('images/tree-rpart.png', dpi = 150)
```
