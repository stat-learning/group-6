...
result in literature is obtained with Convolutional Neural Nets (99.77%). Using our knowledge from this course, we want to build classifiers to recognize what digit (0-9) is displayed in a given image. We are interested in seeing how our models would perform with respect to literature results.
...
We computed the Euclidean distance from each input to its mean image. We plot the sample-to-mean distance distribution for each digit. 
...
The tree plot shows that the important pixels (at each split) seem to be near the center of the image (around 400) (Figure 7). The misclassification rate is near 40% (36.52%), as this is a rather weak model with a limited numer of splits.
